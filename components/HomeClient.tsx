    ...
    >
      <AnimatePresence mode="wait">
        <motion.div
          ...
          className="absolute inset-0 w-full h-full group-hover:scale-125 transition-transform duration-1000"
        >
          <Image
            src={projects[currentHighlight]?.thumbnail || "/placeholder.jpg"}
            fill
...
interface HomeClientProps {
  ...
  serverProfile: any;
  serverEducation: any[];
  serverExperience: any[];
  serverSkills: any[];
  serverCertifications: any[];
  serverContacts: any[];
}

export default function HomeClient({
  serverProjects, serverProfile, serverEducation, serverExperience, serverSkills, serverCertifications, serverContacts
}: HomeClientProps) {
  ...
  const dbProjects = serverProjects;
  const dbProfile = serverProfile;
  const dbEducation = serverEducation;
  const dbExperience = serverExperience;
  const dbSkills = serverSkills;
  const dbCertifications = serverCertifications;
  const dbContacts = serverContacts;
        ...
        <div className="w-full space-y-8 p-6 md:p-16 bg-[#0a0a0a]">
          {project.content?.map((item: any, i: number) => {
            ...
            if (item.type === "image") return (
              <motion.div initial={{ scale: 0.9, opacity: 0 }} whileInView={{ scale: 1, opacity: 1 }} transition={{ duration: 0.8 }} key={i} className="w-full rounded-2xl my-10 border border-white/5 shadow-2xl overflow-hidden relative">
                <Image src={item.value} width={1000} height={600} className="w-full h-auto" alt="Article Content" />
              </motion.div>
            );
    ...
    <div className="relative min-h-screen selection:bg-white selection:text-black bg-[#050505] text-white overflow-x-hidden w-full font-sans">
      ...
      >
        <div className="flex items-center gap-3 cursor-pointer group" onClick={() => scrollTo("about")}>
          <motion.div whileHover={{ rotate: 180, scale: 1.2 }} transition={{ duration: 0.6 }} className="w-8 h-8 md:w-10 md:h-10 relative">
            <Image src="/logo-navbar.png" alt="Logo" fill className="object-contain" />
          </motion.div>
        </div>
      ...
      <main className="relative z-10 w-full">
        ...
        <section id="about" className="min-h-screen flex flex-col justify-between px-4 relative overflow-hidden pt-32 pb-0">
          ...
          <motion.div
            ...
            className="w-full max-w-3xl md:max-w-4xl mt-[-5vh] relative mx-auto flex justify-center items-end flex-grow h-[50vh] md:h-[75vh]"
          >
            ...
            <motion.div whileHover={{ scale: 1.05, y: -20, filter: "brightness(1.2)" }} transition={{ duration: 0.8, type: "spring" }} className="absolute inset-0 w-full h-full z-10 grayscale-[30%] hover:grayscale-0 shadow-2xl">
              <Image
                src={pData.photo || "/placeholder.jpg"} alt="Profile"
                fill
                priority
              ...
              {eduData.map((edu: any, i: number) => (
                ...
                >
                  ...
                  <motion.div className="absolute top-0 right-0 p-12 opacity-10 group-hover:opacity-40 group-hover:scale-150 group-hover:-rotate-12 transition-all duration-1000 pointer-events-none z-0">
                    {edu.logo && <div className="w-48 h-48 relative"><Image src={edu.logo} fill sizes="192px" className="object-contain grayscale" alt="logo" /></div>}
                  </motion.div>
          ...
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ duration: 1.5 }} viewport={{ once: true, amount: 0.05 }} className="w-max flex animate-marquee gap-8 items-center mb-10 hover:!animation-play-state-paused py-4">
            {[...skillData, ...skillData].map((skill: any, i: number) => (
              <div key={i} className="w-48 h-56 md:w-64 md:h-72 shrink-0 border border-white/10 hover:border-white/50 rounded-[2rem] md:rounded-[2.5rem] bg-[#0a0a0a] hover:bg-[#1a1a1a] flex flex-col items-center justify-center p-6 md:p-8 group transition-all duration-500 relative overflow-hidden cursor-default shadow-lg hover:scale-110 hover:-translate-y-5 hover:rotate-2">
                ...
                <div className="w-16 h-16 md:w-20 md:h-20 mb-4 md:mb-6 z-10 relative">
                  <Image src={skill.logo} alt={skill.name} fill sizes="80px" className="object-contain grayscale opacity-60 group-hover:grayscale-0 group-hover:opacity-100 group-hover:scale-125 group-hover:rotate-12 transition-all duration-700" />
                </div>
                    ...
                    <div className="w-full relative overflow-hidden aspect-[4/5] sm:aspect-auto bg-[#111]">
                      <div className="w-full h-full relative min-h-[300px]">
                        <Image
                          ...
                          className="object-cover transition-transform duration-700 ease-out group-hover:scale-110 group-hover:-rotate-2 opacity-90 group-hover:opacity-100"
                          alt={project.title}
                        />
                      </div>
              ...
              {contactsData.map((item: any, i: number) => (
                ...
                >
                  <div className="w-20 h-20 md:w-24 md:h-24 flex items-center justify-center rounded-2xl bg-white/5 border border-white/10 group-hover:bg-white/20 group-hover:scale-110 group-hover:rotate-6 transition-all duration-500 shadow-xl shrink-0">
                    {item.isLocal ? (
                      <div className="w-8 h-8 md:w-10 md:h-10 relative"><Image src={item.imgSrc} alt={item.label} fill sizes="40px" className="object-contain filter invert-0 group-hover:invert transition-all duration-300" /></div>
                    ) : (
                      item.icon && <item.icon size={28} className="text-white drop-shadow-[0_0_10px_white] md:w-[36px] md:h-[36px]" />
                    )}
                  </div>
